...
class BridgeService:
    ...
    def _get_webami_targets(self, upcs: Optional[list[str]]) -> list[dict]:
        if upcs:
            results = []
            for u in upcs:
    ...
    def _push_price(self, sp: dict, new_price: float) -> None:
        variants = sp.get("variants") or []
        self.client.update_product({
            "id": sp["product_id"],
            "variants": [{"id": v["id"], "price": str(new_price)} for v in variants],
        })

    # ── Gap fill helpers ──────────────────────────────────────────────

    def _get_shopify_targets(self, product_ids: Optional[list[str]]) -> list[dict]:
        if product_ids:
            return [p for pid in product_ids if (p := db.get_shopify_product(pid))]
        return db.search_shopify_products("")

    def _compute_gap_updates(self, sp: dict, wp: dict) -> dict:
        updates = {}

        if not sp.get("image_urls"):
            images = wp.get("image_urls") or []
            if images:
        ...
        if not sp.get("product_type") and wp.get("format"):
            ...
            updates["productType"] = (
                ...
                if fmt else "Media > Music & Sound Recordings"
            )
